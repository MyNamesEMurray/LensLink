...
site/
  ...
  wrangler.jsonc    deploy config: dist/ as an assets-only Worker
...
## Deploying (Cloudflare, free)

The domain is already on Cloudflare and the site is static, so hosting
costs nothing on either of the two routes below. **Workers** is the one
the dashboard now steers you to, and it is what `wrangler.jsonc` in this
directory is for.

### Workers (Workers Builds)

`wrangler.jsonc` declares an assets-only Worker — no Worker script, just
the files `build.py` writes into `dist/`.

1. **Workers & Pages → Create → Import a repository**, and pick this
   repository.
2. Build settings:
...
3. **Create and deploy.** The first build publishes to
   `<name>.workers.dev`.
4. **Settings → Domains & Routes → Add → Custom domain** → `lenslink.cam`,
   then again for `www.lenslink.cam`. Because the zone is already in this
   Cloudflare account, the DNS records are created for you and TLS is
   issued automatically.

The build image preinstalls Python 3.13, so `python3 build.py` needs no
setup step; the generator imports nothing outside the standard library.

### Pages

Still supported, and it needs no `wrangler.jsonc` at all: **Workers &
Pages → Create → Pages → Connect to Git**, then production branch `main`,
framework preset None, build command `python3 build.py`, output directory
`dist`, root directory `site`. Add the custom domains the same way.

Either route rebuilds on every push to `main` and gives each pull request
a preview URL.

### What is already configured in the repo

- `wrangler.jsonc` — assets-only Worker: `dist/` as the asset directory,
  `404.html` served for unknown paths, and folder index files served at
  their trailing-slash URL (which is what `build.py` emits).
- `static/_headers` — security headers and cache lifetimes, honoured
  natively by both Workers and Pages. The CSP allows `api.github.com`,
  which the download page calls to list the current release's files;
...
either `npx wrangler deploy` from `site/`, or drag `site/dist` onto
