...
The video appears within a second or two. iOS stops camera capture for
an app that isn't on screen, so leaving LensLink or locking the phone
ends the stream and blanks the source. On iPads that support multitasking
camera access, the stream keeps running beside another app in Split View,
Slide Over and Stage Manager.
