...
- `BroadcastExtension/SampleHandler.swift` is the ReplayKit
  ...
  Its pipeline, `BroadcastExtension/ScreenStreamPipeline.swift`, reuses
  `StreamClient`/`VideoEncoder` and speaks the same protocol with
  `kind: "screen"` (system audio only, no mic, by design). It is the
  only screen mirror on every iOS version: an in-app ScreenCaptureKit
  mirror was tried for iOS 27 and removed, because iOS suspended the
  app as soon as it left the screen.
