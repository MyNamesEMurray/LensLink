    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text("**Keep streaming in the background** moves the picture into a Picture in Picture window when you leave LensLink mid-stream, which is what lets the camera keep running — iOS stops capture for an app that is nowhere on screen. Tap the window for its controls: one button returns you to LensLink, the other closes it. Closing ends the stream, and so does locking the phone. The window shows the picture the way OBS receives it — sensor-native landscape — however you are holding the phone. Pushing the window off the side of the screen parks it, and iOS only lends the camera to a window it can see: capture pauses, the green camera dot goes out, and OBS holds its last frame. Pull the window back and the stream resumes on its own. That one is iOS's rule, not a setting. You can also stop from anywhere without the window: press Stop in OBS or the web panel, or say \"Stop streaming with LensLink\". On iPads that support it the camera keeps running in Split View, Slide Over and Stage Manager with no window needed, and screen mirroring keeps running across apps regardless.")
