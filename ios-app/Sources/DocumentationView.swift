    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text("**Keep streaming in the background** moves the picture into a Picture in Picture window when you leave LensLink mid-stream, which is what lets the camera keep running — iOS stops capture for an app that is nowhere on screen. Close the little window and the stream ends; lock the phone and it ends too. On iPads that support it the camera also keeps running in Split View, Slide Over and Stage Manager, with no PiP window needed. Screen mirroring is unaffected: a broadcast keeps running across apps regardless.")
