...
/// the app is inline.
///
/// **Why the video-call content source, and not the sample-buffer one.**
/// The sample-buffer flavour
/// (`ContentSource(sampleBufferDisplayLayer:playbackDelegate:)`) is the
/// one that draws close and restore buttons, which this flavour has not
/// had since iOS 16. It was tried, in v1.11.3, and the window never
/// opened at all: background streaming broke outright. Apple documents
/// camera-in-PiP alongside this controller, and device testing agrees —
/// so the window here has no buttons of its own, and stopping from
/// outside the app goes through OBS, the web panel or Siri. Don't swap
/// the flavour again without a device to prove the window still opens.
///
...
extension BackgroundPiP: AVPictureInPictureControllerDelegate {
    ...
    nonisolated func pictureInPictureController(
        ...
        // Logged rather than swallowed: a window that never opens is the
        // one PiP failure with no symptom of its own — the stream simply
        // ends as the app leaves the screen, exactly as it did before
        // background streaming existed. The reason iOS gives is the only
        // way to tell "this device won't" from "this build asked wrongly".
        print("[LensLink] PiP failed to start: \(error.localizedDescription)")
